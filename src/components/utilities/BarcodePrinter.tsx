...
import { Barcode, Printer, X, Loader2 } from 'lucide-react';
...
export default function BarcodePrinter({ product, onClose }: BarcodePrinterProps) {
  ...
  async function handlePrint() {
    ...
    const labelsHtml = labels.map(l => {
      const barcodeSvg = generateCode128(l.barcode);
      return `
        <div class="label" style="width:${dims.w}mm;height:${dims.h}mm;display:flex;flex-direction:column;align-items:center;justify-content:center;page-break-inside:avoid;border:0.5px dashed #ccc;padding:2mm;">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="space-y-2">
            ...
            {allBarcodes.map(b => {
              ...
              return (
                <div key={id} className={`flex items-center gap-3 p-3 rounded-lg border transition-all cursor-pointer ${selected ? 'border-primary-300 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-100 dark:border-gray-800'}`} onClick={() => toggleBarcode(id)}>
